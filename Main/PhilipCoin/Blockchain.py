...
class BlockChain:
  ...
  def transaction(self, sender, reciever, amount):
    previus_block=self.chain[-1]
    tr=sender.submit_transaction(amount, reciever)
    if len(previus_block.data) == 2 or len(self.chain) == 1:    
      self.pool.append(previus_block)
      self.chain.append(Block(previus_block.index+1, date.datetime.now(), tr, previus_block.hash_block()))
    else:
